...
Мы начинаем сравнивать выборки желая узнать, есть ли между ними различия. 
Иногда мы не строим предположения заранее. Где крупнее ящерицы, под Берлином или под Саратовом? Кто знает?

В других случаях мы имеем ясные ожидания: число излечившихся, мы надеемся, будет больше при использовании нового лекарства, чем старого. 

## Гипотезы: нулевая и альтернативная

Первый шаг в сравнении – формулировка нулевой гипотезы.

Положение $Н_0$ привелигерованное: именно ее мы тестируем, принимаем или отвергаем.

И чаще всего она формулируется как отсутствие различий между сравниваемыми объектами. Скажем, что ящерицы из Германии и Поволжья одинаково крупные.
...
Решение о том, принять или отвергнуть $H_0$ мы принимаем после статистического теста. Но результат теста не тождественен реальному состоянию дел - он не скажет, верна ли $H_0$, только укажет, как с ней поступить.

Вне зависимости от нас и наших действий, реальность может находиться в одном из двух состояний:  $H_0$ верна, ящерицы одинаковы, либо $H_0$ неверна, и ящерицы различаются.

Таким образом, возможно четыре исхода теста.

В мире где ящерицы одинаковы мы можем 

- принять $H_0$ (и это будет верное решение),
- либо отвергнуть ее (и это будет ошибка). 

Аналогично, в мире где ящерицы различаются, мы можем 

- принять $H_0$ (что ошибочно),
...
| 	|$H_0$ == TRUE |	$H_0$ == FALSE |
|-----|-----|-----|
| Отклонить $H_0$ | Ошибка I рода с вероятностью <span class="orange">&alpha;</span></br>Ложно-положительный результат | 	Верно |
| Сохранить $H_0$ | Верно | Ошибка II рода с вероятностью <span class= "blue">&beta;</span> </br> Ложно-отрицательный результат |
<br /><br />
...
Вероятность ошибки первого рода мы задаем сами --- это уровень значимости теста, $\alpha$. В биологии используют 5% и реже 1%.

Это та вероятность, меньше которой мы отказываемся верить в справедливость $H_0$.

Ошибка второго рода обычно скрыта. Но ее можно уменьшить вслепую, например увеличив объем выборки.

Интересно, что справедлива следующая формулировка:
Если в результате теста мы приняли $H_0$, но она ошибочна с вероятность $\beta$, если же мы отвергли $H_0$, то она верна с вероятностью $\alpha$.

## Как принять решение?

### Статистические таблицы
Принцип прост -- мы вычисляем значение тестовой статистики, и сравниваем его с критическим значением этой же статистики из таблицы.
...
p-value это вероятность - вероятность того, что наблюдаемые различия между выборками возникли случайно.

Если р велико, выборки принципиально одинаковы, если p принимает маленькие значения - выборки принципиально различаются.
...
P-value = 0.05 не значит, что $H_0$ верна с вероятностью 5%!

P-value = 0.05 значит, что в мире, где все ящерицы одинаковы а $H_0$ верна, шанс получить результат который мы получили составляет 5%.
...
В R есть 4 полезных функции для работы с распределениями: `p, q, d, r`.

- `r` = random number generation.

С помощью этой функции можно смоделировать взятие выборки из генеральносй совокупности с заданными параметрами.

Например: `rnorm(1000, mu = 20, sd = 2)` сгенерирует выборку в 1000 случайных значений из нормального распределения.
...
Эта опрерация принципиально обратна тому, что делает `q`.
...
Эта функция позволяет определить высоту кривой плотности вроятностей для данного х. Если передать ей последовательность значений, можно смоделировать кривую плотности вероятности. Давайте так и сделаем, а потом построим график.
...
```{r, sim.norm.ggplot, fig.height=4.5}
...
Для начала создадим две выборки из популяций с нормальным распределением величин и заведомо отличающимися значениями $\mu$
...
## Разделим столбцы гистограммы по признаку gender и сохраним в новую переменную
...
## Посчитаем средние для двух уровней фактора gender

Создадим два логических вектора

```{r}
gender_m <- df_height$gender == "M"
gender_f <- df_height$gender == "F"
```

Рассчитаем средние

```{r}
mean_m <- mean(df_height$height[gender_m])
mean_f <- mean(df_height$height[gender_f])
mean_m
mean_f
```
...
```{r}
SE_f <- sd(df_height$height[gender_f])^2 / sum(gender_f)
SE_m <- sd(df_height$height[gender_m])^2 / sum(gender_m)
```  

Подставим значения в формулу t-статитстики

```{r}
t.value <- (mean_f - mean_m) / sqrt(SE_f + SE_m) 
t.value
```
...
Как видим, различия достоверны.

## Проверим себя

```{r}
t_height <- t.test(data = df_height, height ~ gender)
# data = df_height задает источник данных
# height ~ gender показывает, что значения переменной
# height сгруппированы по признаку gender
...
### Вопрос: Вероятность какого события отражает p=`r t_height$p.value`?
...
Это вероятность того, что в мире, где $H_0$ верна, шанс получить эмпирические выборки с такими различиями как у нас, составляет 0.2%.
...
Прочитав описание, вы должны были заметить, что не все измерения в таблице равнозначны.

Есть несколько случаев, в которых нам неизвестна действительная продолжительность ремиссии, потому что пациент вышел из программы, или потому что время исследований закончилось.

Это должно было напомнить вам историю о ленивых лягушках.

Если бы мы считали медианы, такие значения ничуть бы нам не повредили. Со средней, к сожалению, этот номер не пройдет. Мы должны распрощаться с этими значениями:

```{r purl=FALSE}
valid <- subset(rem, rem$cens == 1)
```

## Решение

Теперь, отобрав пригодные для анализа данные, можем провести t-тест.
...
Если мы отвергаем $Н_0$, имеются различия.

Если мы не отвергаем $Н_0$, все одинаково. 
