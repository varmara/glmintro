...
4. Одна из градаций фактора (первая, если не специально не указать иное) становится **базовым уровнем**
...
```{r}
model.matrix(M1) %*% coefficients(M1)
...
Если мы хотим вычислить предсказание модели (значение объема легких) для индавида, имеющего Group =  Short exposed
...
$$
 ...
 = 4.46205
$$
Это базовый уровень. Для него предсказанное занчение равно `Intercept`
...
Необходимо включать еще одну перменную - **`ковариату`**
...
Какой фактор ставить на какое место?
...
- Нет разницы между моделями с дискретнми и непрерывными предикторами
