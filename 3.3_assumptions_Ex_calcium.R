...
# логарифмируем time и создаем новую переменную в датафрейме с данными
...
gg_resid <- ggplot(data = M2_diag, aes(x = .fitted, y = .stdresid)) +
  geom_point() + geom_hline(yintercept = 0)
gg_resid
# Видна небольшая гетерогенность дисперсий

# 3) Графики остатков от предикторов в модели и не в модели
gg_resid + aes(x = time_l)
# Видна небольшая гетерогенность дисперсий
...
# Итог: Нелинейность мы убрали, но осталась небольшая гетерогенность дисперсий.
