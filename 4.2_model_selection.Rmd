...
M5_diag <- fortify(M5)
removed_vars <- c("lcp", "lbph", "age", "pgg45", "svi", "gleason")
M5_diag_full <- data.frame(M5_diag, prost[, removed_vars])
ggplot(M5_diag_full, aes(x = 1:nrow(M5_diag_full), y = .cooksd)) + 
  geom_bar(stat = "identity")
```
...
gg_resid <- ggplot(data = M5_diag_full, aes(x = .fitted, y = .stdresid)) + 
  geom_point() + geom_hline(yintercept = 0)
gg_resid
```
...
- по-одному `anova(M1, mod2, test = "Chisq")`
- все сразу `drop1(M1, test = "Chisq")`
