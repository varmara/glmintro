...
```{r purl=FALSE, eval=TRUE, fig.width=10, echo=FALSE}
library(ggplot2)
library(gridExtra)
grid.arrange(
...
>- Хорошо бы добавить точки наблюдений
...
```{r, fig.height=2}
ggplot(goat, aes(x = wt, y = 1:nrow(goat))) + geom_point()
ggplot(goat, aes(x = init, y = 1:nrow(goat))) + geom_point()
```
...
```{r, fig.show='hold', fig.height=3, fig.width=10, purl=FALSE}
set.seed(9827)
cook_cutoff <- 4 / (nrow(goat) - length(coef(MG1) - 2))
op <- par(mfrow = c(1, 3)) # располагаем картинки в 3 колонки
plot(MG1, which = 4, cook.levels = cook_cutoff)  # Расстояние Кука
residualPlot(MG1)        # График остатков
qqPlot(MG1)              # Квантильный график остатков
par(op)
```
...
```{r, fig.show='hold', fig.height=3.5, fig.width=10, purl=FALSE}
set.seed(849)
cook_cutoff <- 4 / (nrow(goat) - length(coef(MG2) - 2))
op <- par(mfrow = c(1, 3)) # располагаем картинки в 3 колонки
plot(MG2, which = 4, cook.levels = cook_cutoff)  # Расстояние Кука
residualPlot(MG2)        # График остатков
qqPlot(MG2)              # Квантильный график остатков
par(op)
```
...
```{r, purl=FALSE}
MG2_diag <- data.frame(goat, .fitted = fitted(MG2), .resid = resid(MG2))
gg_res <- ggplot(MG2_diag, aes(x = .fitted, y = .resid)) + 
...
gg_res %+% aes(x = init)
gg_res %+% aes(x = treat) + geom_boxplot()
```

```{r eval=TRUE, echo=FALSE, purl=FALSE, fig.width=10}
grid.arrange(
gg_res %+% aes(x = init),
gg_res %+% aes(x = treat) + geom_boxplot(),
nrow = 1)
