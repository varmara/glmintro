...
fev[which(!complete.cases(fev)), ] #здесь используется сразу две функци which(), в нее вложена complete.cases(); ! - логическая операция "NOT". [] - оператор индексации 


fev <- fev[complete.cases(fev), ]  #Так мы отобрали только те строки, которые не содержат NA 
...
#############################################################################
#Визуализация данных (Geoms)
#Геомы - это геометрические объекты, которые отражают данные
...
# Поскольку в основе построения гистограммы лежит статистическая процедура (определение частот), то все, то же самое можно сделать с помощью функций stat_...

ggplot(fev, aes(x = FEV)) + stat_bin(geom = "bar") #Аналогично!

ggplot(fev, aes(x = FEV)) + stat_bin(geom = "bar", binwidth = 0.1) #Аналогично!


# Но! частотное распределение можно выразить еще и линией

ggplot(fev, aes(x = FEV)) + stat_bin(geom = "line", size = 1, color = "red")

ggplot(fev, aes(x = FEV)) + stat_bin(geom = "line", size = 1, color = "red", binwidth = 1)
...
ggplot(fev, aes(x = FEV)) + geom_histogram() + facet_wrap( ~ Sex, ncol = 1) #Так удобнее сравнивать

#geom_violin() 
#удобный геом для отражения частотных распределений
ggplot(fev, aes(x= Sex, y = FEV)) + geom_violin(aes(fill = Sex)) 


# Отражаем частотное распределение с использованием данных по полу и курению
ggplot(fev, aes(x = FEV, fill = Smoker)) + geom_histogram() + facet_wrap( ~ Sex, ncol = 1) #Трудно читать
...
#############################################################################
#Визуализация данных (Простейшая статистическая обработка)
...
# Можно сразу вписать простые линейные модели и увидеть характер связей
